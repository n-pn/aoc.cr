...
# reduce shape to list of integer
SHAPES = PROTOS.map &.map { |(x, y)| x * WIDTH + y }

def top_rows(screen, height, rows = 30) # can be 25, but 30 to make sure
  String.build do |str|
...
remain_iter = 1000000000000 - start_idx
cycle_count = remain_iter // cycle_length

puts max_heights[2022]
