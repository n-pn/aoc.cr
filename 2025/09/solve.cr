...
def part2(input : Array(String))
  ...
  arr.each_cons_pair do |(ax, ay), (bx, by)|
    if ax == bx
      y_map[ay].to(y_map[by]).each { |i| inner << {x_map[ax], i} }
    else
      x_map[ax].to(x_map[bx]).each { |i| inner << {i, y_map[ay]} }
    end
  end
  ...
  arr.each_with_index do |(ax, ay), i|
    arr[i + 1..].each do |bx, by|
      next unless x_map[ax].to(x_map[bx]).all? { |x| inner.includes?({x, y_map[ay]}) }
      next unless x_map[ax].to(x_map[bx]).all? { |x| inner.includes?({x, y_map[by]}) }
      next unless y_map[ay].to(y_map[by]).all? { |y| inner.includes?({x_map[ax], y}) }
      next unless y_map[ay].to(y_map[by]).all? { |y| inner.includes?({x_map[bx], y}) }
