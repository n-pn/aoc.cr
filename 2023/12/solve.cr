...
def solve(text : String, ints : Array(Int32))
  CACHE.put_if_absent({text, ints}) do
    ...
    sum1 = char.in?('.', '?') ? solve(text[1..], ints) : 0_i64
    break sum1 if char == '.' || !text[0..left].matches?(/^[?#]+[?.]$/)

    sum1 + solve(text[(left + 1)..], right)
  end
end
...
def part2(text)
  text.lines.sum do |line|
    str, ints = line.split(' ', 2)
    solve([str].*(5).join('?') + '.', ints.split(',').map(&.to_i) * 5)
  end
end
